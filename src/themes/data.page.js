	...
	for(const page of pages){
		...
		for(const id of api){
				...
				if(code in hexjson.hexes){
					...
					/*if("units" in page[id]){
						for(let c in page[id].units){
							if(c in data.data.constituencies[code]){
								if("scale" in page[id].units[c] || "precision" in page[id].units[c]){
									let v = new Decimal(data.data.constituencies[code][c]);
									if("scale" in page[id].units[c]){
										// Scale the number
										data.data.constituencies[code][c] *= page[id].units[c].scale;
										v = v.times(page[id].units[c].scale);
									}
									if("precision" in page[id].units[c]){
										// Round to nearest x
										v = v.div(page[id].units[c].precision).toInteger().times(page[id].units[c].precision);
									}
									data.data.constituencies[code][c] = v.toNumber();
								}
							}
						}
					}*/
			...
			if("units" in page[id]){
				...
				/*for(let c in data.units){
					delete data.units[c].scale;
					delete data.units[c].precision;
				}*/
			}
