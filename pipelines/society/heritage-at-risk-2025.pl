...
my @rows = LoadCSV($basedir.'../../raw-data/HAR Register 2025 Entries including additions and removals.csv',{
	'columns'=>{
...
foreach $pcon (sort(keys(%{$constituencies}))){
	...
	if($pid =~ /^E/){
		...
		$csvfull .= ",".($mplookup->{$pid}{'MP'}||"?");
		$csvfull .= ",".($mplookup->{$pid}{'Party'}||"?").",".updatePartyName($mplookup->{$pid}{'Party'});
		$csvfull .= ",".$mplookup->{$pid}{'ID'};#($mplookup->{$pid}{'MPID'} ? "https://members.parliament.uk/member/$mplookup->{$pid}{'MPID'}/contact" : "");
...
# Save outputs
$file = $basedir."../../src/themes/society/heritage-at-risk/_data/HAR2025.csv";
...
sub updatePartyName {
	...
	elsif($party eq "RUK"){ $party = "Reform"; }
